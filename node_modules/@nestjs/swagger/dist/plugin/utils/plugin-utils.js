...
exports.getAbsoluteCompilerOptionsPaths = getAbsoluteCompilerOptionsPaths;
...
function replaceImportPath(typeReference, fileName, options, compilerOptionsPaths) {
    ...
    try {
        ...
        for (const [optionName, optionPaths] of Object.entries(compilerOptionsPaths)) {
...
function getAbsoluteCompilerOptionsPaths(program) {
    const compilerOptions = program.getCompilerOptions();
    const { paths } = compilerOptions;
    if (!paths) {
        return {};
    }
    const base = compilerOptions.pathsBasePath ||
    ...
    for (const [key, list] of Object.entries(paths)) {
        result[key.replace('/*', '')] = list.map((item) => {
            ...
            return (0, path_1.isAbsolute)(path) ? path : path_1.posix.join(base, path);
        });
    }
    return result;
}
